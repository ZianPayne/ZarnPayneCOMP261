...
public class Stop implements Comparable<Stop> {
    ...
    //Field to record the depth of node
    private int depth = -1;
    ...
    //  outNeighbours returns a collection of the stops that are connected forward out of this stop. Used in components
    //  backNeighbours returns a collection of stops that are connected backwards out of a stop. Used in components 
    //--------------------------------------------
    ...
    /** Get the collection of outNeighbouring stops*/
    public Collection<Stop> getForwardNeighbours() {
        Collection<Edge> outEdges = getForwardEdges();
        Set<Stop> outStops = new HashSet<>();
        
        for ( Edge e : outEdges){
            outStops.add(e.toStop());
        }
        
        return Collections.unmodifiableSet(outStops);
    }
         
    /** Get the collection of backNeighbouring stops*/
    public Collection<Stop> getBackwardNeighbours() {
        Collection<Edge> backEdges = getBackwardEdges();
        Set<Stop> backStops = new HashSet<>();
        
        for ( Edge e : backEdges){
            backStops.add(e.fromStop());
        }
        
        return Collections.unmodifiableSet(backStops);
    }
    ...
    //---------------------------------------------
    // fields and methods for finding articulation points
    //---------------------------------------------

    public void setStopDepth(int d){
        this.depth = d;
    }
    
    public int getStopDepth() {
        return depth;
    }
}
